...
for sys in cstx2_kv42 cstx3_kv42 cstx2_kv43 cstx3_kv43; do
  for rep in prod_rep1 prod_rep2 prod_rep3 prod_rep4 prod_rep5 prod_rep6 prod_rep7 prod_rep8; do
    [ -f "$sys/${rep}_ss.dat" ] && { echo "[$sys] $rep hbonds/ss done, skip"; continue; }
    echo "=========================================================="
    echo "[$sys] $rep hbonds+ss  $(date +%T)"
    $MM run -p "$ENV" python 06_analyze_hbonds_ss.py "$sys" "$rep" "${NTOX[$sys]}" 2>&1 | grep -vE "Warning|deprecat"
  done
done
echo ">>> HBONDS+SS ALL DONE  $(date)"
